...
# If manual timestamps are requested
...
raspi_only_files = []
extended_files = []
recap_data = []  # << ajout pour le fichier récap
...
if csv_files_raspi:
    for csv_file_raspi in csv_files_raspi:
        ...
        df_Vdots.to_excel(writer, sheet_name="Vdot_raw", float_format="%.5f")
        df_data_raspi.to_excel(writer, sheet_name="RasPi", float_format="%.5f")
        writer.close()
        raspi_only_files.append((f'cfm_analysis_{csv_file_raspi.name.split(".")[0]}.xlsx', output_raspi.getvalue()))

        # --- Ajout au fichier recap (au minimum p_mean + Vdot_stats) ---
        recap_entry = {"file": csv_file_raspi.name, "p_mean": df_p, "Vdot_stats": df_Vdot_stats}
        ...
        if (df_GM_CR_raw_global is not None) or (df_GM_GR_raw_global is not None):
            ...
            if GM_stats_dict:
                ...
                df_Vdot_stats.to_excel(writer, sheet_name="Vdot_stats", float_format="%.5f")
                df_Vdots.to_excel(writer, sheet_name="Vdot_raw", float_format="%.5f")
                df_GM_stats.to_excel(writer, sheet_name="CO2_stats", float_format="%.9f")
                if "CR" in GM_stats_dict: df_GM_CR.to_excel(writer, sheet_name="CO2_CR", float_format="%.9f")
                if "GR" in GM_stats_dict: df_GM_GR.to_excel(writer, sheet_name="CO2_GR", float_format="%.9f")
                df_data_raspi.to_excel(writer, sheet_name="RasPi", float_format="%.5f")
                writer.close()
                extended_files.append((f'cfm_analysis_extended_{csv_file_raspi.name.split(".")[0]}.xlsx', output_ext.getvalue()))

                # Ajout au recap
                recap_entry["CO2_stats"] = df_GM_stats

        recap_data.append(recap_entry)
...
if raspi_only_files:
    ...
    st.download_button("Download all the results (RasPi seul)", zip_buffer_raspi.getvalue(), "results_raspi_only.zip", mime="application/zip")
...
if extended_files:
    ...
    with zipfile.ZipFile(zip_buffer_ext, "w") as zf:
        for fname, fdata in extended_files:
            zf.writestr(fname, fdata)
    st.download_button("Download all the results (étendu)", zip_buffer_ext.getvalue(), "results_extended.zip", mime="application/zip")
...
if recap_data:
    ...
    writer = pd.ExcelWriter(recap_output, engine="xlsxwriter")
